...
@section('content')
<form method="POST" action={{ route('ilumukita.login') }}>
   @csrf
    <div class="form-group">
      <label class="small mb-1" for="input_login_email">Email</label>
      <input name="email" class="form-control py-4 @error('email') is-invalid @enderror" id="input_login_email" type="email" placeholder="Enter email address" autocomplete="email" value="{{ old('email') }}"/>
      @error('email')
